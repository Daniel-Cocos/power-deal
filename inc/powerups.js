...
function createPowerupSVG(title) {
  ...
  function arc(cx, cy, rx, ry, startAngle, endAngle, fill, strokeWidth = 1, stroke = 'none') {
  const a = document.createElementNS(svgNS, 'path');
  const d = `M ${cx + rx * Math.cos(startAngle)} ${cy + ry * Math.sin(startAngle)} 
             A ${rx} ${ry} 0 0 1 ${cx + rx * Math.cos(endAngle)} ${cy + ry * Math.sin(endAngle)}`;
  a.setAttribute('d', d);
  a.setAttribute('fill', fill);
  ...
  switch (title) {
    ...
    case 'Remove': {
      // Shadow
      ellipse(120, 125, 45, 10, '#00000033');

      // Card
      rect(100, 60, 40, 60, '#fff', 5);
      text('♣', 120, 100, '#000', 32);

      // Glow
      path('M100,60 h40 v60 h-40 Z', 'none', '#00ffff', 2); // Cyan glow

      // Minus symbol
      text('–', 140, 77, '#ff0000', 50, 'bold');
...
function showPopup(data, powerupElement) {
  popupImage.src = data.image || ''; // Optional image
  popupDescription.textContent = data.description;
  powerupPopup.style.display = 'flex';

  // Generate and insert the SVG
  const popupSVGContainer = document.getElementById('popupSVGContainer');
  popupSVGContainer.innerHTML = ''; // Clear previous SVG
  const svg = createPowerupSVG(data.title);
  svg.setAttribute('width', '200');
  svg.setAttribute('height', '150');
  popupSVGContainer.appendChild(svg);
...
// Powerups
const powerups = [
  { title: 'Sturdy Wipe', description: 'All active powerups get ignored', chance: 1, type: 'Utility' },
  { title: 'Shield', description: 'Used to prevent any powerup to be used on you', chance: 1, type: 'Defensive' },
  { title: 'Mirror', description: 'Redirect offensive powerup back to user', chance: 1, type: 'Defensive' },
  { title: '+5', description: 'Add 5 to self or opponent', chance: 1, type: 'Offensive' },
  { title: '+10', description: 'Add 10 to self or opponent', chance: 1, type: 'Offensive' },
  { title: 'Remove', description: 'Remove latest drawn card', chance: 1, type: 'Offensive' },
  { title: 'Force Draw', description: 'Force opponent to draw another card', chance: 1, type: 'Offensive' },
  { title: 'Swap Hands', description: 'Swap hands with opponent', chance: 1, type: 'Utility' },
  { title: 'Rewind', description: 'Revert last action', chance: 1, type: 'Utility' },
  { title: 'Steal', description: 'Steal last drawn card', chance: 1, type: 'Offensive' },
  { title: 'Signal Jammer', description: 'Block opponent powerups next round', chance: 1, type: 'Offensive' },
  { title: 'Invisible Hand', description: 'Your cards hidden next round', chance: 1, type: 'Defensive' },
  { title: 'Raised Stakes', description: 'Force everyone to raise bets by up to 3 including yourself', chance: 1, type: 'Offensive' },
  { title: 'Chicken', description: 'Remove 1 chip from your bet', chance: 1, type: 'Defensive' },
];
